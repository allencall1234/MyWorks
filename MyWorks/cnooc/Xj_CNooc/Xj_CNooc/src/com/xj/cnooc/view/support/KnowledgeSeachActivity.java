 ...
 * 2015-9-24
 */
public class KnowledgeSeachActivity extends BaseActivity implements IXListViewListener,OnClickListener{
...
//	private RelativeLayout rellayout_back;
	...
	Handler handler=new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				progress=ProgressDialog.show(KnowledgeSeachActivity.this,"", "正在加载数据,请稍候...");
				UserHttpBiz.getKnowledgeBseListData(curragePage, num, et_seach.getText().toString(), new HttpDataCallBack() {
					
					@Override
					public void HttpSuccess(String _result) {
						progress.dismiss();
						adapter=new KnowledgeBaseListAdapter(KnowledgeSeachActivity.this, getData(_result));
						if(totalNum==0){
							Toast.makeText(KnowledgeSeachActivity.this, "没有找到与关键词相关的信息", 1).show();
						}
						if(totalNum <= 10){
					...
					public void HttpFail(int ErrCode) {
						progress.dismiss();
						Toast.makeText(KnowledgeSeachActivity.this, "网络请求失败,请检查您的网络！", 1).show();
					}
				});
				break;
			...
			case 2:
				UserHttpBiz.getKnowledgeBaseDetailData(knowledgeType,knowledgeId,new HttpDataCallBack(){

					@Override
					public void HttpSuccess(String _result) {
						// TODO Auto-generated method stub
						if(knowledgeType.equals("1") || knowledgeType.equals("2")){
							try {
								...
								if(knowledgeType.equals("1")){
									...
									faultSupportModelDetail.setSuperName(jsonObject.getJSONObject("hitchSupport").getString("superName"));
									faultSupportModelDetail.setCompeten_advice(jsonObject.getJSONObject("hitchSupport").getString("competen_advice"));
									faultSupportModelDetail.setCompeten_grade(jsonObject.getJSONObject("hitchSupport").getString("competen_grade"));
									faultSupportModelDetail.setConclusion(jsonObject.getJSONObject("hitchSupport").getString("conclusion"));
									faultSupportModelDetail.setApprovalContent(jsonObject.getJSONObject("hitchSupport").getString("approvalContent"));
									faultSupportModelDetail.setApprovalUserName(jsonObject.getJSONObject("hitchSupport").getString("approvalUserName"));
								...
								if(knowledgeType.equals("2")){
									faultSupportModelDetail.setTitle(jsonObject.getJSONObject("assessmentSupport").getString("title"));
									faultSupportModelDetail.setDescription(jsonObject.getJSONObject("assessmentSupport").getString("description"));
									faultSupportModelDetail.setCreateTime(jsonObject.getJSONObject("assessmentSupport").getString("createTime"));
									String type=jsonObject.getJSONObject("assessmentSupport").getString("supportType");
									faultSupportModelDetail.setType(type);
									faultSupportModelDetail.setCreaterName(jsonObject.getJSONObject("assessmentSupport").getString("createrName"));
									faultSupportModelDetail.setElectricNetName(jsonObject.getJSONObject("assessmentSupport").getString("electricNetName"));
									
									faultSupportModelDetail.setElectricRunTypeName(jsonObject.getJSONObject("assessmentSupport").getString("electricRunTypeName"));
									faultSupportModelDetail.setSupportSortName(jsonObject.getJSONObject("assessmentSupport").getString("supportSortName"));
									faultSupportModelDetail.setFactoryUserName(jsonObject.getJSONObject("assessmentSupport").getString("factoryUserName"));
									faultSupportModelDetail.setProficientName(jsonObject.getJSONObject("assessmentSupport").getString("proficientName"));
									faultSupportModelDetail.setSuperName(jsonObject.getJSONObject("assessmentSupport").getString("superName"));
									faultSupportModelDetail.setCompeten_advice(jsonObject.getJSONObject("assessmentSupport").getString("competen_advice"));
									faultSupportModelDetail.setCompeten_grade(jsonObject.getJSONObject("assessmentSupport").getString("competen_grade"));
									faultSupportModelDetail.setConclusion(jsonObject.getJSONObject("assessmentSupport").getString("conclusion"));
									faultSupportModelDetail.setApprovalContent(jsonObject.getJSONObject("assessmentSupport").getString("approvalContent"));
									faultSupportModelDetail.setApprovalUserName(jsonObject.getJSONObject("assessmentSupport").getString("approvalUserName"));
								...
								for (int i = 0; i < jsonArray.length(); i++) {
									...
									pingYiModel.setCreaterName(jsonArray.getJSONObject(i).getString("createrName"));
									pingYiModel.setContent(jsonArray.getJSONObject(i).getString("content"));
									pingYiModel.setCreateTime((jsonArray.getJSONObject(i).getString("createTime")));
									pingYiModel.setIsAdopted((Integer.parseInt(jsonArray.getJSONObject(i).getString("isAdopted"))));
									list.add(pingYiModel);
								}

								JSONArray jsonArray1=jsonObject.getJSONArray("ticklingList");
								List<FeedBackModel> feedBackModels=new ArrayList<FeedBackModel>();
								for (int i = 0; i < jsonArray1.length(); i++) {
									FeedBackModel feedBackModel=new FeedBackModel();
									feedBackModel.setCreateName(jsonArray1.getJSONObject(i).getString("createrName"));
									feedBackModel.setContent(jsonArray1.getJSONObject(i).getString("content"));
									feedBackModel.setCreateTime((jsonArray1.getJSONObject(i).getString("createTime")));
									feedBackModels.add(feedBackModel);
								}
								
								Intent intent=new Intent();
								intent.setClass(KnowledgeSeachActivity.this, KnowledgeBaseDetailActivity.class);
								Bundle bundle=new Bundle();
								bundle.putSerializable("falutSupportData", faultSupportModelDetail);
								bundle.putSerializable("pingyiList", (Serializable)list);
								bundle.putSerializable("feedbackList", (Serializable)feedBackModels);
								bundle.putSerializable("attachmentList", (Serializable)attachmentBeans);
								bundle.putString("type", knowledgeType);
								intent.putExtras(bundle);
								KnowledgeSeachActivity.this.startActivity(intent);
							} catch (JSONException e) {
								e.printStackTrace();
							}
						}
						
						if(knowledgeType.equals("3") || knowledgeType.equals("4")){
//							Toast.makeText(KnowledgeBaseActivity.this, "暂不支持该类型！", 1).show();
							...
							try {
								JSONObject jsonObject=new JSONObject(_result);
								videoTitle=jsonObject.getJSONObject("record").getString("title");
								createTime=jsonObject.getJSONObject("record").getString("createTime");
								type=jsonObject.getJSONObject("record").getString("type");
								
								JSONArray jsonArray=jsonObject.getJSONArray("attachmentList");
								for (int i = 0; i < jsonArray.length(); i++) {
									attachmentPath =jsonArray.getJSONObject(i).getString("path");
									attachmentName =jsonArray.getJSONObject(i).getString("name");
									createName = jsonArray.getJSONObject(i).getString("creatername");
								}
							...
							Intent intent=new Intent();
							intent.setClass(KnowledgeSeachActivity.this, VideoKnowledgeBaseDetailActivity.class);
							Bundle bundle=new Bundle();
							bundle.putString("videoTitle",videoTitle);
							bundle.putString("createName", createName);
							bundle.putString("createTime", createTime);
							bundle.putString("type", type);
							bundle.putString("attachmentPath", attachmentPath);
	...
	private void init(){
		...
		refreshListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				final KnowledgeBaseModel knowledgeBaseModel=knowledgeBaseModels.get(position-1);
...
//			break;
		...
		case R.id.btn_seach:
			if(et_seach.getText().toString()==null || et_seach.getText().toString().equals("")){
				Toast.makeText(KnowledgeSeachActivity.this, "请输入查询关键字", 0).show();
			}else{
				new Thread(new Runnable() {
...
//		}.execute(new Void[]{});
	...
	private List<Map<String, Object>> getData(String _result){
		...
		knowledgeBaseModels=new ArrayList<KnowledgeBaseModel>();
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				createTime = "发布时间："+jsonArray.getJSONObject(i).getString("createTime");
				...
				knowledgeBaseModel.setId(Integer.parseInt(jsonArray.getJSONObject(i).getString("id")));
