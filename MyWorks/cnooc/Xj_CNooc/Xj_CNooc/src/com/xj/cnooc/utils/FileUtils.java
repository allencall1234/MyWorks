...
public class FileUtils
{
	...
	public FileUtils(ProgressBar downProgress, Context context, ImageButton ib_download)
	{
...
//	}
	 ...
	 * @describe : TODO
	 */
	public void asyncDownloadFile(final String fileUrl, final String fileName, final String type)
	{
		new Thread(){
			@Override
			public void run()
	...
	{  
        ...
        {
        	flag = 1;
//        	 Toast.makeText(context, fileName + "已存在", Toast.LENGTH_SHORT).show();
        }  
        else
        {  
           try 
           {  
        	   ...
        	   URL url = new URL(HttpURL.SERVICE_URL + "phoneDownload.html?downLoadPath="+str_url+"&fileName="+str_fileName+"&type="+type);
...
//	   		    String[] fileUrl_array = (HttpURL.SERVICE_URL + urlStr).split("\\/");
//	   			String file_name = fileUrl_array[fileUrl_array.length - 1].split("\\?")[0];
//   			    File file = null;
...
//   由于得到一个InputStream对象是所有文件处理前必须的操作，所以将这个操作封装成了一个方法  
   ...
   {       
        ...
        {  
    		...
    		{
    			totalDownSize = urlConn.getContentLength();//将要下载的文件的总大小保存到全局变量 中
		...
		public void handleMessage(android.os.Message msg) 
		{
			switch (msg.what) 
			{
				...
				case 0:
					...
					Toast.makeText(context, "下载成功,请到downloadFile文件夹中查看下载文件!", Toast.LENGTH_SHORT).show();
					break;
				case -1:
					Toast.makeText(context, "下载失败,文件不存在或网络连接异常!", Toast.LENGTH_SHORT).show();
					break;
				case 1:
					ib_download.setVisibility(View.GONE);
					downProgress.setProgress(downProgress.getMax());
					Toast.makeText(context, "文件已存在,请到downloadFile文件夹中查看下载文件!", Toast.LENGTH_SHORT).show();
					break;
				}
		};
	};
