 ...
 * 2015-9-18
 */
public class FinishFaultDetailActivity extends BaseActivity implements OnClickListener{
...
//		btn_setting.setOnClickListener(this);
		...
		if(type.equals("故障详情")){
			...
			finish_tv_originator.setText("发起人："+faultSupportModel.getCreaterName());
			finish_tv_faultClassify.setText("故障分类："+faultSupportModel.getSupportSortName());
			finish_tv_faultType.setText("故障类型："+faultSupportModel.getType());
			finish_tv_belongto.setText("所属电网："+faultSupportModel.getElectricNetName());
			finish_tv_runWay.setText("电网运行方式:"+faultSupportModel.getElectricRunTypeName());
			if(faultSupportModel.getType().equals("一般")){
				finish_tv_support_manufacturer.setText("支持厂家："+faultSupportModel.getFactoryUserName());
			}
			if(faultSupportModel.getType().equals("严重") || faultSupportModel.getType().equals("隐患")){
				finish_tv_support_manufacturer.setText("支持专家："+faultSupportModel.getProficientName());
			}
			finish_tv_support_expert.setVisibility(View.GONE);
			finish_tv_treatment_measures_detail.setText("        "+faultSupportModel.getDescription().trim());
		...
		if(type.equals("评估详情")){
			...
			finish_tv_originator.setText("发起人："+faultSupportModel.getCreaterName());
			finish_tv_faultClassify.setText("评估分类："+faultSupportModel.getSupportSortName());
			finish_tv_faultType.setText("评估类型："+faultSupportModel.getType());
			finish_tv_belongto.setText("所属电网："+faultSupportModel.getElectricNetName());
			finish_tv_support_expert.setText("支持专家："+faultSupportModel.getProficientName());
			finish_tv_runWay.setText("电网运行方式:"+faultSupportModel.getElectricRunTypeName());
		...
		adapter = new FinishPingYiAdapter(FinishFaultDetailActivity.this, listItems);
...
//			break;
		...
		case R.id.finish_btn_download_attachment:
			intent = new Intent(FinishFaultDetailActivity.this, DownloadDetailsActivity.class);
...
//			intent=new Intent();
//			intent.setClass(FinishFaultDetailActivity.this, SettingActivity.class);
//			startActivity(intent);
//			break;
