...
public class AnyChatVideoHelper {
	...
	public int bindVideo(SurfaceHolder holder) {
		...
		// }
		// }
		render[0] = new VideoRenderer(holder);
		index = 0;
		return index;
	}
...
class VideoRenderer implements Callback {
	...
	public Bitmap CreateBitmap(int width, int height) {
		...
		if (bitmap != null
				&& (srcRect.bottom != height || srcRect.right != width))
			bitmap = null;
		if (bitmap == null) {
		...
		if (canvas != null) {
			...
			if (rotation != 0) {
				...
				if (rotation == 90 || rotation == 270) {
					temp_b_w = b_h;
					temp_b_h = b_w;
					matrix.postTranslate(1.0f / 2 * (b_h - b_w),
							1.0f / 2 * (b_w - b_h));
				}
			}
