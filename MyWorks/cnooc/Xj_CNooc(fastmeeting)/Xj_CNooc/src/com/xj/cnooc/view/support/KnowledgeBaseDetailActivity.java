 ...
 *         2015-9-24
 */
public class KnowledgeBaseDetailActivity extends BaseActivity implements
		OnClickListener {
	...
	private void init() {
		...
		Serializable feedBackSerializable = bundle
				.getSerializable("feedbackList");
		if (feedBackModels == null && feedBackSerializable != null) {
			...
			for (int i = 0; i < feedBackModels.size(); i++) {
				...
				map.put("feed_back_name", feedBackModels.get(i).getCreateTime()
						+ "  " + feedBackModels.get(i).getCreateName());
		...
		if (type.equals("1")) {
			...
			knowledge_base_tv_faultClassify.setText("故障分类："
					+ faultSupportModel.getSupportSortName());
			knowledge_base_tv_faultType.setText("故障类型："
					+ faultSupportModel.getType());
			if (faultSupportModel.getType().equals("一般")) {
				knowledge_base_tv_support_manufacturer.setText("支持厂家："
						+ faultSupportModel.getFactoryUserName());
			}
			if (faultSupportModel.getType().equals("严重")
					|| faultSupportModel.getType().equals("隐患")) {
				knowledge_base_tv_support_manufacturer.setText("支持专家："
		...
		if (type.equals("2")) {
			...
			knowledge_base_tv_faultClassify.setText("评估分类："
					+ faultSupportModel.getSupportSortName());
			knowledge_base_tv_faultType.setText("评估类型："
					+ faultSupportModel.getType());
			knowledge_base_tv_support_expert.setVisibility(View.GONE);
			knowledge_base_tv_support_manufacturer.setText("支持专家："
					+ faultSupportModel.getProficientName());
		}
		knowledge_base_tv_originator.setText("发起人："
				+ faultSupportModel.getCreaterName());
		knowledge_base_tv_belongto.setText("所属电网："
				+ faultSupportModel.getElectricNetName());
		knowledge_base_tv_runWay.setText("电网运行方式："
				+ faultSupportModel.getElectricRunTypeName());
		knowledge_base_tv_support_expert.setVisibility(View.GONE);
		knowledge_base_tv_treatment_measures_detail.setText("        "
		...
		knowledge_base_tv_date.setText("发布时间："
				+ faultSupportModel.getCreateTime());
		...
		if (faultSupportModel.getCompeten_grade().equals("")
				|| faultSupportModel.getCompeten_grade() == null) {
			ratingBar.setRating(0);
		} else {
			ratingBar.setRating(Integer.parseInt(faultSupportModel
					.getCompeten_grade()));
		}
		tv_knowledge_success_result_content.setText(faultSupportModel
				.getCompeten_advice());

		adapter = new FinishPingYiAdapter(KnowledgeBaseDetailActivity.this,
				listItems);
		listView.setAdapter(adapter);

		feedBackAdapter = new FeedBackAdapter(KnowledgeBaseDetailActivity.this,
				feedBacklistItems);
	...
	public void onClick(View v) {
		...
		case R.id.knowledge_base_btn_download_attachment:
			// Toast.makeText(KnowledgeBaseDetailActivity.this,
			// ""+faultSupportModel.getPath(), 1).show();
			intent = new Intent(KnowledgeBaseDetailActivity.this,
		...
		// intent=new Intent();
		// intent.setClass(KnowledgeBaseDetailActivity.this,
