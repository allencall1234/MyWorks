...
public class HttpUntilImpl {
	...
	public static void getStringFromUrl(final String url,
			final HttpDataCallBack callBack) {
				...
				try {
					...
					if (status == 200) {
						...
						mHandler.obtainMessage(0,
								new HttpDataCallBackEntity(result, callBack))
								.sendToTarget();
						return;
					}
					mHandler.obtainMessage(
							1,
							new HttpDataCallBackEntity(String.valueOf(status),
									callBack)).sendToTarget();
				} catch (Exception e) {
					try {
						mHandler.obtainMessage(1,
								new HttpDataCallBackEntity("-404", callBack))
								.sendToTarget();
					} catch (Exception e2) {
