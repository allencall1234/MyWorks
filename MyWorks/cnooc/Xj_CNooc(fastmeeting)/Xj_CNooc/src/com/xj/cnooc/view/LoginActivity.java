...
public class LoginActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (UserHelper.getInstance() != null) {
			String useranme = UserHelper.getInstance().getSharedPreferences()
					.getString(INTENT_USERNAME, "");
			String password = UserHelper.getInstance().getSharedPreferences()
	...
	public void onClick(View v) {
		...
		case R.id.btn_login:
			...
			if (TextUtils.isEmpty(userName) || TextUtils.isEmpty(password)) {
				Toast.makeText(LoginActivity.this, R.string.login_error_isNull,
				...
				UserHttpBiz.Login(userName, password, new HttpDataCallBack() {
					...
					public void HttpFail(int ErrCode) {
						Toast.makeText(LoginActivity.this, "网络请求失败,请检查您的网络!",
								Toast.LENGTH_SHORT).show();
					}
				});
			}
		...
		public void handleMessage(Message msg) {
			...
			try {
				if (obj.getString("message").equals("0")) {
					...
					UserBean bean = (UserBean) MyUtils.putJsonToObject(object,
							"com.xj.cnooc.model.UserBean");
					MyApp.globelUser = bean;

					if (UserHelper.getInstance() != null) {
						System.out.println(login_et_username.getText()
								.toString()
								+ login_et_password.getText().toString());

						Editor editor = UserHelper.getInstance()
								.getSharedPreferences().edit();
						editor.putString(INTENT_USERNAME, login_et_username
								.getText().toString());
						editor.putString(INTENT_PASSWORD, login_et_password
								.getText().toString());
						editor.commit();
					}
