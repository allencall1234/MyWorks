...
/**
 * 
 * @author Administrator 点击帖子列表进入到的帖子详情界面Activity
 *
 */
public class PostDetailsActivity extends Activity implements OnClickListener,
	...
	protected void initData() {
		// post_position = getIntent().getIntExtra("post_position",
		// post_position);// 获取点击的帖子下标
	...
	private void initView() {
		...
		ImageLoader.getInstance().loadImage(
				HttpURL.SERVICE_URL + postBean.getPhoto(),
		...
		lv_reply_list.setOnTouchListener(new View.OnTouchListener() {
	...
	private Handler handler = new Handler(new Handler.Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			case 0:
				...
				UserHttpBiz.queryReplyPostList(curragePage, num, postId,
						loginId, new HttpDataCallBack() {
							@Override
							public void HttpSuccess(String _result) {
								...
								adapter = new CommentedPostListViewAdapter(
										getReplyPostData(_result), context);
			...
			case 1:
				...
				break;
			case 2:
				UserHttpBiz.collectionPost(postId, loginId,
								...
								try {
									...
									if (object.getBoolean("status") == true) {
										if (object.getInt("message") == 0) {
											Toast.makeText(context, "此帖子收藏成功",
													Toast.LENGTH_SHORT).show();
											MyCollectionFragment.collection_list
													.add(postBean);
				...
				break;
			case 3:
				UserHttpBiz.savePostReplyContent(postId, send_message, loginId,
						new HttpDataCallBack() {
							@Override
							public void HttpSuccess(String _result) {
								...
								try {
									JSONObject object = new JSONObject(_result);
									if (object.getBoolean("status") == true) {
										reply_list.add(new ReplyPostBean(
												MyApp.globelUser.getName(),
												send_message, MyUtils.getTime()));
										reply_total = reply_total + 1;
										tv_right_title.setText(String
												.valueOf(reply_total + "条回帖"));
										adapter.notifyDataSetChanged();
										setListViewHeightBasedOnChildren(lv_reply_list);
										...
										// Toast.makeText(context, "保存回复内容成功",
										// Toast.LENGTH_SHORT).show();
									}
									// finish();
								} catch (JSONException e) {
			...
			case 4:
				...
				// if(bean.getName().endsWith(".jpeg")
				// ||bean.getName().endsWith(".jpg") ||
				// bean.getName().endsWith(".png"))
				// {
				//
			...
			if (jsonObject.getBoolean("status") == true) {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					JSONObject jsonObject2 = jsonArray.getJSONObject(i)
							.getJSONObject("bbsPostReply");

					ReplyPostBean bean = (ReplyPostBean) MyUtils
							.putJsonToObject(jsonObject2,
									"com.xj.cnooc.model.ReplyPostBean");
				...
				if (jsonArray2.length() == 1) {
					...
					AttachmentBean bean = (AttachmentBean) MyUtils
							.putJsonToObject(jsonObject2,
									"com.xj.cnooc.model.AttachmentBean");
