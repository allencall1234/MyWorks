...
/**
 * 
 * @author Administrator 各个技术讨论区的Adapter
 *
 */
public class BBSListViewAdapter extends BaseAdapter {
	...
	public BBSListViewAdapter(Context context, List<BBSBean> list,
			int request_code) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			viewHolder.tv_discussion_name = (TextView) convertView
					.findViewById(R.id.tv_discussion_name);// 技术讨论区名称
			viewHolder.tv_moderator = (TextView) convertView
					.findViewById(R.id.tv_moderator);// 版主
			viewHolder.tv_discussion_introduce = (TextView) convertView
					.findViewById(R.id.tv_discussion_introduce);// 技术讨论区介绍
			viewHolder.tv_post_time = (TextView) convertView
					.findViewById(R.id.tv_post_time);// 发帖时间
			viewHolder.tv_post_account = (TextView) convertView
					.findViewById(R.id.tv_post_account);// 发帖数量
		...
		// viewHolder.tv_num.setText(String.valueOf(bean.getOneModuleTotal()));//
		// 设置未读数量
		// }
		...
		for (int i = 0; i < list.size(); i++) {
			if (position == i && bean.getOneModuleTotal() > 0) {
				if (request_code != 0x07) {
					viewHolder.layout_count.setVisibility(View.VISIBLE);
					viewHolder.tv_num.setText(String.valueOf(bean
							.getOneModuleTotal()));// 设置未读数量
